#Example 4: Create a Spark script that filters for temp
...
maxTemps = rdd.filter(lambda x: 'TMAX' in x[1]) #--> This will return  --> ('EZE00100082', 'TMAX', 3.6)
stationTemps = maxTemps.map(lambda x: (x[0],x[2])) #--> This will return  --> ('EZE00100082' 3.6)
maxTemps = stationTemps.reduceByKey(lambda x,y: max(x,y))  #--> This will return the min value for each key
