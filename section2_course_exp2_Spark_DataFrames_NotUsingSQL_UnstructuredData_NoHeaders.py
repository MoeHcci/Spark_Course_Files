...
# 4. Running Pandas commands (based on SQL) directly
schemaPeople.groupby("age").count().orderBy("age").show()
    # NOTES:
        # We did not need to use spark.sql, we directly ran commands on the dataFrame
        # The course chanined the operations. We do not always have to do that
        # show() similar to pandas accepts a specific number of rows

# 5. The Spark session must be stopped so the next time this script runs it again getOrCreate a spark session
