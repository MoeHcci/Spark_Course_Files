...
for key, val in results_1:
    if key in results_1__d:
        # the round is used to round the values to 2 decimal points
        results_1__d[key] = round(results_1__d[key] + val,2) #equal to the previous results plus the new value
    else:
        # the round is used to round the values to 2 decimal points
        results_1__d[key] = round(val, 2)
